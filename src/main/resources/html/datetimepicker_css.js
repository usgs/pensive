...
function Calendar(pDate, pCtrl)
{
	...
	this.Month = pDate.getUTCMonth();//selected month number
	this.Year = pDate.getUTCFullYear();//selected year in 4 digits
	this.Hours = pDate.getUTCHours();
...
function RenderCssCal(bNewCal)
{
	...
	for (j = 1; j <= Cal.GetMonDays(); j += 1)
	{
		...
		else if ((j === dtToday.getUTCDate()) && (Cal.Month === dtToday.getUTCMonth()) && (Cal.Year === dtToday.getUTCFullYear()))
		{
			strCell = GenCell(j, true, TodayColor);//Highlight today's date
		}
		else
		{
			if ((j === selDate.getUTCDate()) && (Cal.Month === selDate.getUTCMonth()) && (Cal.Year === selDate.getUTCFullYear())){
	...
	if (Cal.ShowTime === true)
	{
		...
		vCalTime = "<tr><td colspan='7' style=\"text-align:center;\"><table border='0' width='199px' cellpadding='0' cellspacing='0'><tbody><tr><td height='5px' ALIGN='right'>Time (UTC)&nbsp;&nbsp;</td>";
		...
		vCalTime += "<td width='22px'><input type='text' name='hour' maxlength=2 size=1 style=\"WIDTH:22px;float:right;\" value=" + showHour + " onkeyup=\"javascript:Cal.SetHour(this.value)\">";
		vCalTime += "</td><td style='font-weight:bold;text-align:center;' width='10px'>:</td><td width='22px'>";
		vCalTime += "<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px;float:left;\" value=" + Cal.Minutes + " onkeyup=\"javascript:Cal.SetMinute(this.value)\">";
